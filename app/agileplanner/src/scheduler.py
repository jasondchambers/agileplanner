...
class TeamScheduler:
    def __init__(self, team_capacity: TeamCapacity, assigned_epics: set[Epic]):
        ...
        self.detailed_capacity_data = copy.deepcopy(team_capacity.detailed_capacity_data)
    ...
    def who_has_capacity(self,day,category_of_work):
        ...
        for person_name in self.detailed_capacity_data[day].keys():
            remaining_capacity = sum(self.detailed_capacity_data[day][person_name][category_of_work])
            if remaining_capacity > 0:
                l.append(person_name)
        return l

    def apply_capacity(self, day, category_of_work, person_name, epic_remaining):
        person_has_available = sum(self.detailed_capacity_data[day][person_name][category_of_work])
        if person_has_available > 0:
            subtract_capacity = min(epic_remaining,person_has_available)
            epic_remaining -= subtract_capacity
            for c in EpicType:
                self.detailed_capacity_data[day][person_name][c.name].append(-subtract_capacity)
    ...
    def schedule_epic(self, epic) -> ScheduleResult:
        ...
        for day in self.detailed_capacity_data.keys():
            ...
            for person in list_of_persons:
                ...
                if epic_remaining > 0:
                    epic_remaining = self.apply_capacity(day, epic.epic_type.name, person, epic_remaining)
            if epic_remaining == 0:
                    epic_end_date = day
